...
$config_file = __DIR__ . '/config.php'; //상위폴더로 변경 . '/../config.php'
            ...
            <div class="btn-group btn-group-sm flex-wrap" role="group">
                ...
                ?>
                    <a class="fw-bold btn btn-outline-primary" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>" class="edit-file">
                        <i class="fa fa-pencil-square"></i> <?php echo lng('DesignEditor') ?>
                    </a>
                    <a class="fw-bold btn btn-outline-primary" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&env=ace"
                        class="edit-file"><i class="fa fa-pencil-square"></i> <?php echo lng('CoreEditor') ?>
                    </a>
                <?php } ?>
    ...
    <div class="path">
        <div class="row">
            ...
            <div class="edit-file-actions col-xs-12 col-sm-7 col-lg-6 text-end pt-1">
                <div class="btn-group">
                    ...
                    <?php if ($is_text) { ?>
                        <?php if ($isNormalEditor) { ?>
                            <a title="Advanced" class="btn btn-sm btn-outline-primary" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&amp;env=ace"><i class="fa fa-pencil-square-o"></i> <?php echo lng('CoreEditor') ?></a>
                        ...
                        <?php } else { ?>
                            <a title="Plain Editor" class="btn btn-sm btn-outline-primary" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>"><i class="fa fa-text-height"></i> <?php echo lng('DesignEditor') ?></a>
		...
		?>
			...
			nhn.husky.EZCreator.createInIFrame({
				oAppRef: oEditors,
				elPlaceHolder: "normal-editor",
				sSkinURI: "/smarteditor2/SmartEditor2Skin_ko_KR.html",
				fCreator: "createSEditor2",
				fOnAppLoad : function(){
					// 에디터 로딩 완료 후 데이터 설정
					var data = document.getElementById("normal-editor").value;
					// 데이터를 줄바꿈(\n) 단위로 배열화
					...
					oEditors.getById["normal-editor"].exec("SET_CONTENTS", [lines.join('\n')]);
				}
			});
			</script>
			-->
            ...
            <div class="col-xs-6 col-sm-7">
                <ul class="navbar-nav justify-content-end" data-bs-theme="<?php echo FM_THEME; ?>">
                    ...
                    <?php if (!FM_READONLY): ?>
						...
						<li class="nav-item">
                            <a title="<?php echo lng('CoreUpdate') ?>" class="nav-link" href="/core/util/core-update.php" onclick="return confirm('Would you like to update the source code in the core folder to the latest version?');"><i class="fa fa-share-alt-square"></i> <?php echo lng('CoreUpdate') ?><span class="new-tag">new</span></a>
                        </li>
						<script src="https://code.jquery.com/jquery-latest.js"></script>
						<script>
							$.ajax({
...
<?php
    ...
    function lng($txt)
    {
        ...
        $tr['en']['DesignEditor']           = 'Design Editor';
