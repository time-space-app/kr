...
└── core/ (홈페이지 빌드의 핵심코드인 티니파일매니저v2.6 오픈소스 위치)
    ├── api/ (게시판과 같은 백엔드 처리 위치)
        ...
        ├── chat-page-api.php(제미나이 기반의 RAG-검색 증강 생성 챗봇을 워드프레스 페이지 URL기반으로 구현한 백엔드 소스)
        ├── chat-secure-api.php(제미나이 기반의 RAG-검색 증강 생성 챗봇을 코어 에디터 기반으로 구현한 백엔드 소스-소스를 수정하는 기능으로 보안때문에 관리자만 사용가능)
    ...
    ├── util/ (빌더 이용 시 사용되는 유틸리티 소스)
        ├── dhwmanage.php(DB관리자 로그인 후 1개의 파일에 DB관리에 대한 모든 기능이 포함되어 있다. 외부오픈소스사용)
    ...
    ├── chat.php(제미나이 기반의 RAG-검색 증강 생성 챗봇을 고정된 위치의 MD파일 기반으로 구현한 UI샘플화면)
    ├── chat-page.php(제미나이 기반의 RAG-검색 증강 생성 챗봇을 워드프레스 페이지 URL기반으로 구현한 UI샘플화면)
    ├── chat-secure.php(제미나이 기반의 RAG-검색 증강 생성 챗봇을 코어 에디터 기반으로 구현한 UI샘플화면-소스를 수정하는 기능으로 보안때문에 관리자만 사용가능)
...
return [
    ...
    'API_KEY' => '구글aistudio.google.com에서발급받은API키',
...
### kr 빌더의 핵심기능(아래)
#### 워드프레스처럼 소스 코어를 버튼으로 업데이트 받는 기능(아래)
...
- 코어 에디터인 Ace에디터에서 마크다운(MardDown) 파일을 열었을 때 미리보기화면 출력기능 추가.
- 디자인파일을 만들기 전 반드시 테마를 선택 후 만들어야 기본 디자인 페이지 외에 원하는 게시판, 지도 및 뉴스레터 기능이 자동연동 됩니다.(현재 3개의 테마가 샘플로 제공 됩니다.)
#### 추가된 최신 기능(아래)
- QR코드가 연동되는 물품관리 기능의 샘플 페이지 추가.
- 메뉴는 템플릿 디자인이 없기 때문에 1개만 별도 제작 후 index테마에 포함(메뉴에 페이지 번역기능 추가).
- 몽9에디터의 템플릿디자인과 연동되는 구글지도맵, 게시판, 메뉴까지 theme 폴더에 기능별로 분리.
- RestAPI+Ajax 방식으로 index테마와 news-letter테마에서 1페이지 CRUD 기능.
- mysql 기반의 1페이지 게시판 기능 및 페이징 기능 추가
- 게시판, 뉴스레터 사용 시 보안위협 코드는 제외하고 필수태그만 출력 가능하도록 보안필터 적용.
- 게시판,뉴스레터 검색기능 추가.
- 홈페이지에 MD문서를 만들고, 해당 MD문서를 가져와 문서기반 검색 증강 생성형(RAG) 챗봇 기능 추가.
- 코어 에디터에 AI 어시스턴트 창을 추가하여 현재 수정중인 페이지에 AI 생성형 코드를 사용할 수 있다.
...
#### Adminer DB매니저 포함(adminer.php명 때문에 dhwmanage.php 로 변경해서 사용함 아래)
- https://www.adminer.org/ (오픈소스인 v5.5.1 사용 LICENSE-2.0 Apache License, GNU General Public License v2.0 배포)
