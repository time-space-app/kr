...
└── core/ (홈페이지 빌드의 핵심코드인 티니파일매니저v2.6 오픈소스 위치)
    ├── api/ (게시판과 같은 백엔드 처리 위치)
        ...
        ├── chat-secure-api.php(제미나이 기반의 RAG-검색 증강 생성 챗봇을 코어 에디터 기반으로 구현한 백엔드 소스-보안때문에 관리자만 사용가능)
    ...
    ├── chat-secure.php(제미나이 기반의 RAG-검색 증강 생성 챗봇을 코어 에디터 기반으로 구현한 UI샘플화면-보안때문에 관리자만 사용가능)
