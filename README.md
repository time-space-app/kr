...
└── core/ (홈페이지 빌드의 핵심코드인 티니파일매니저v2.6 오픈소스 위치)
    ...
    ├── module/ (게시판과 같은 기능 추가 위치)
        ├── _footer-basic.php(몽9에디터가 포함된 디자인 에디터에서 저장시 기본디자인만 추가되는 푸터파일로 이것을 복사해 새로은 기능을 추가할 수 있다.)
        ├── _footer-index.php(몽9에디터가 포함된 디자인 에디터에서 저장시 자동으로 추가되는 메뉴와 지도기능의 푸터파일에 포함된 모듈)
        ├── _footer-news-letter.php(몽9에디터가 포함된 디자인 에디터에서 저장시 자동으로 추가되는 뉴스레터 기능페이지의 푸터파일)
        ├── _header-basic.php(몽9에디터가 포함된 디자인 에디터에서 저장시 기본디자인만 추가되는 헤더파일로 이것을 복사해 새로운 기능을 추가할 수 있다.)
        ├── _header-index.php(몽9에디터가 포함된 디자인 에디터에서 저장시 자동으로 추가되는 RestAPI게시판 기능의 헤더파일에 포함된 모듈)
...
├── basic.php (티니파일매저에서 basic테마로 선택 후 제공된 몽9에디터로 제작된 샘플 디자인페이지)
├── index.php (티니파일매저에서 light테마로 선택 후 제공된 몽9에디터로 제작된 샘플 메인-게시판페이지)
├── index-temp.html (제공된 페이지준비중 안내 페이지)
├── LICENSE (타임스페이스 kr빌더의 GPL-3.0 라이센스내용)
├── news-letter.php (티니파일매저에서 dark테마로 선택 후 제공된 몽9에디터로 제작된 뉴스레터 게시판연동 샘플 페이지)
